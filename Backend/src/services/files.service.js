...
class FilesService {
    ...
    async PostVideo(body, files, user) {
        const { id } = user
        const { title } = body
        const { file } = files
    ...
    async GetVideoInfo(params) {
        ...
        const FoundUser = await pool.query("select f.title , f.size, f.created_at , f.file_name,json_build_object('id',u.id,'name',u.username,'avatar',u.avatar) as user from files as f inner join users as u on f.user_id = u.id where u.id  = $1", [id])
    ...
    async SearchVideo(params) {
        const { data } = params
        const JoinData = await pool.query("select f.title , f.size, f.created_at , f.file_name,json_build_object('id',u.id,'name',u.username,'avatar',u.avatar) as user from files as f inner join users as u on f.user_id = u.id where u.username ilike $1 or f.title ilike $2", [`%${data}%`, `%${data}%`])
    ...
    async GetAllFiles() {
        const Allfiles = await pool.query("select f.title , f.size, f.created_at , f.file_name,json_build_object('id',u.id,'name',u.username,'avatar',u.avatar) as user from files as f inner join users as u on f.user_id = u.id ")
        if (Allfiles.rows.length === 0) {
            throw new NotFoundError(404, "empty")
        }
    ...
    async GetInfoByToken(user) {
        const { id } = user
        const FoundUser = await pool.query("select f.id,f.title , f.size, f.created_at , f.file_name,json_build_object('id',u.id,'name',u.username,'avatar',u.avatar) as user from files as f inner join users as u on f.user_id = u.id where u.id  = $1", [id])
